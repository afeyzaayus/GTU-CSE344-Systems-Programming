...
static double calc_high_priority_score(t_shm *shm, t_args *args){
    ...
    while (1){
        if (shm->d->count > 0){
            ...
            pthread_cond_signal(&shm->d->not_full);
...
typedef struct s_hp_args {
    t_shm   *shm;
    t_args  *args;
    double  result;
} t_hp_args;
 
static void *hp_thread_func(void *arg){
    t_hp_args *hparg = (t_hp_args *)arg;
    hparg->result = calc_high_priority_score(hparg->shm, hparg->args);
    return (NULL);
}
...
void run_aggregator(t_args *args, t_shm *shm)
{
    ...
    hp_args.args   = args;
    hp_args.result = 0.0;
    if (pthread_create(&hp_tid, NULL, hp_thread_func, &hp_args) != 0)
        perror("pthread_create hp_thread");
    for (int i = 0; i < 4; i++) {
    ...
    double hp_score = hp_args.result;
